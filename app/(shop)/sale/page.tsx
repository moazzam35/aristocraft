...
function SalePageContent() {
  ...
  useEffect(() => {
    ...
    fetch("/api/products?limit=200")
      ...
      .then((data) => {
        if (cancelled) return;
        // Filter discountPercentage > 0, sort by discountPercentage desc by default
        const saleItems = [...(data.products || [])]
      ...
      .catch(() => {
        ...
        setError(true);
        setLoading(false);
        showToast("Failed to load sale products.", "error");
      });
    return () => {
      cancelled = true;
    };
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    switch (sort) {
      ...
      default:
        // 'featured' keeps the initial discountPercentage desc sorting
        return list;
    }
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 md:px-16 pt-16 sm:pt-20 md:pt-24 pb-20 md:pb-28">
        ...
        >
          <p className="text-sm font-light" style={{ color: "#1C1C1C99" }}>
            {loading
              ...
              : `${filteredAndSorted.length} ${filteredAndSorted.length === 1 ? "sale item" : "sale items"}`}
          </p>
        ...
        {!loading && error && (
          <p className="text-sm py-12 text-center" style={{ color: "#1C1C1C99" }}>
            Couldn&rsquo;t load sale items right now. Please refresh to try again.
          </p>
        )}
        ...
        {!loading && !error && filteredAndSorted.length === 0 && (
          ...
          >
            ...
            <p className="text-base font-medium" style={{ color: "#1C1C1C" }}>
              No sale items match your criteria
            </p>
