...
function NewArrivalsPageContent() {
  ...
  useEffect(() => {
    ...
    fetch("/api/products?limit=200")
      ...
      .then((data) => {
        if (cancelled) return;
        // Sort all products by ID descending and take first 12 as new arrivals
        const sortedNew = [...(data.products || [])].sort((a, b) => b.id - a.id).slice(0, 12);
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    switch (sort) {
      ...
      default:
        // 'featured' keeps the initial ID desc sorting
        return list;
    }
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 md:px-16 pt-16 sm:pt-20 md:pt-24 pb-20 md:pb-28">
        ...
        >
          <p className="text-sm font-light" style={{ color: "#1C1C1C99" }}>
            {loading
              ...
              : `${filteredAndSorted.length} ${filteredAndSorted.length === 1 ? "arrival" : "arrivals"}`}
          </p>
        ...
        {!loading && error && (
          <p className="text-sm py-12 text-center" style={{ color: "#1C1C1C99" }}>
            Couldn&rsquo;t load new arrivals right now. Please refresh to try again.
          </p>
        )}
        ...
        {!loading && !error && filteredAndSorted.length === 0 && (
          ...
          >
            ...
            <p className="text-sm" style={{ color: "#1C1C1C66" }}>
              Try clearing search or filters to see more
            </p>
        ...
        {!loading && !error && filteredAndSorted.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-12 md:gap-x-8 md:gap-y-16">
            {filteredAndSorted.map((product, i) => (
              <ProductCard key={product.id} product={product} index={i} showNewBadge={true} />
            ))}
          </div>
        )}
      </div>
